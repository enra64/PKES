...
#define NUMBER_OF_INIT_VALUES 1000

class Integrator{
  GlidingAverage _buffer;
  int16_t _angle;

  int16_t _bufferInitCounter = NUMBER_OF_INIT_VALUES;
  int32_t _initSum = 0;
  ...
  void processAngleDelta(){
    ...
    //avg *= 5;
    //avg /= 133*133;
    //zu langsam, werte ganz ok
    //avg *= 6; // magic number
    //avg /= _sensitivity * 10;// avg / sens * (1 / 0.096s)
    //avg *= 9; //24magic number
    //avg /= _sensitivity * 37;//37=(1 / 0.027s) 37*133 = 
    avg /= _sensitivity * 55;//65 war ganz gut
    //avg /= 2;
    _angle += avg;
  }
  ...
  void injectGyroData(int16_t axisAdc){
    ...
    if(_bufferInitCounter <= 0 && abs(axisAdc) < IMU_ADC_READING_MINIMUM_ACCEPTED_VALUE)
      axisAdc = 0;
  ...
  int16_t getAngle(){
    ...
    if(_bufferInitCounter > 0) return -1;
    return _angle * 2;
  }
  ...
  void reset(){
    ...
    _buffer.clear();
    _initSum = 0;
    _bufferInitCounter = NUMBER_OF_INIT_VALUES;
  ...
  bool createReference(){
    if(_bufferInitCounter > 0)
      return false;
    _angle = 0;
    _offset = _initSum / NUMBER_OF_INIT_VALUES;

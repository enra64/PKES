#define IMU_OCR_VALUE 250 //current filter has 2.8 ms delay, so all 3 ms should be the best value
//250->1kHz|1ms, so 500->500Hz|2ms
#define IMU_TIMER_MS_DELTA_PER_TICK (IMU_OCR_VALUE / 25)
...
void setup() {
  ...
  initImuTimer();
  Serial.print("ctc timer init ok, ms/tick: ");
  Serial.println(IMU_TIMER_MS_DELTA_PER_TICK);
  ssdInit();
...
void requestTargetAngle(){
  ...
  transition(SPEED, MOTOR_RIGHT, 2);
} 

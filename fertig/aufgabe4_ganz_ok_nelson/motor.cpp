...
void Motor::init(void) {
    ...
    TCCR4B |= (1 << CS41) | (1 <<
                             WGM42);//| (1 << WGM43);//fast pwm, mode 7. using mode 15 we can change the top value, not the output voltage
...
void Motor::setRight(int16_t value) {
        setMotor(abs(value), value < 0 ? REW_2 : FOR_2);
}

void Motor::setLeft(int16_t value) {
        setMotor(abs(value), value < 0 ? REW_1 : FOR_1);
}
...
#define BRAKE_SPEED_R_MARGE -150
#define BRAKE_SPEED_L_MARGE -150

#define BRAKE_SPEED_R_KRUSTY 200
#define BRAKE_SPEED_L_KRUSTY 200

#define BRAKE_SPEED_R_NELSON 300
#define BRAKE_SPEED_L_NELSON 300

#define BRAKE_SPEED_R BRAKE_SPEED_R_NELSON
#define BRAKE_SPEED_L BRAKE_SPEED_L_NELSON

void Motor::forceBrake() {
    if(_direction2 == REW_2)
        setRight(BRAKE_SPEED_R);
    else if(_direction2 == FOR_2)
        setRight(-BRAKE_SPEED_R);
    else
        Serial.println("right no brake");

    if(_direction1 == REW_1)
        setLeft(BRAKE_SPEED_L);
    else if(_direction1 == FOR_1)
        setLeft(-BRAKE_SPEED_L);
    else
        Serial.println("left no brake");
    _delay_ms(30);
    drive(0);
}
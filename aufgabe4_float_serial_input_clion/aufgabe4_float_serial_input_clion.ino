...
#define TURNTICKS_KRUSTY 100
#define TURN_TARGET_TICKS TURNTICKS_KRUSTY

#define TURN_SPEED_MARGE 200
#define TURN_SPEED_KRUSTY 300

#define TURN_SPEED_KRUSTY_TWO_WHEEL 200

#define TURN_SPEED TURN_SPEED_KRUSTY_TWO_WHEEL
...
void execOdoTurnEvent(int16_t targetTicks){
    mOdometry.setTarget(MOTOR_LEFT, TURN_TARGET_TICKS);//marge
    mOdometry.setTarget(MOTOR_RIGHT, TURN_TARGET_TICKS);//marge
    uint32_t currentTicks = mOdometry.getCurrentTicks(WHAT_ODOMETRY_WORKS);
    ...
    checkForAbort(targetTicks, currentTicks, TURN);
}
...
void execTurnEvent(int16_t ticks) {
    ...
    if(WHAT_ODOMETRY_WORKS == MOTOR_RIGHT){
        ...
        mMotorControl.setRight(ticks > 0 ? -TURN_SPEED : TURN_SPEED);
        mMotorControl.setLeft(ticks > 0 ? TURN_SPEED : -TURN_SPEED);
    }
...
void batchControl() {
    while (!mInput.isFinished()) {
        ...
        switch (event.type) {
            ...
            case 't':
            default:
                execOdoTurnEvent(event.parameter);
        }
    }
}

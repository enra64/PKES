...
private:
  ...
  AverageRing mRightRing, mLeftRing;
public:
  uint32_t getCurrentTicks(uint8_t isRight);
  void init();
  void hit(uint8_t isRight);
  float ticksPerSecond(uint8_t motor);
  int16_t calculatePwmTarget(uint8_t motor, uint8_t output);
  int16_t getTarget(uint8_t motor);
  void setTarget(uint8_t isRight, int16_t target);
  void reset();
};
